"""
효주는 포도주 시식회에 갔다. 그 곳에 갔더니, 테이블 위에 다양한 포도주가 들어있는 포도주 잔이 일렬로 놓여 있었다. 효주는 포도주 시식을 하려고 하는데
...
예를 들어 6개의 포도주 잔이 있고, 각각의 잔에 순서대로 6, 10, 13, 9, 8, 1 만큼의 포도주가 들어 있을 때, 첫 번째, 두 번째, 네 번째, 
다섯 번째 포도주 잔을 선택하면 총 포도주 양이 33으로 최대로 마실 수 있다.
...
"""
dp [i] => i번째까지 먹엇을때 최대 와인 양

6 

6 10

6 10 13 max(wine[0] + wine[1], wine[1] + win[2], wine[0] + wine[2])

6 10 13 9 
9 안 마셨을때 = dp[3]
9와 인접한거 마셨을때 = wine[i] + wine[i-1] + dp[i-3]
9와 인접한거 안마셨을때 = wine[i] + dp[i-2]
